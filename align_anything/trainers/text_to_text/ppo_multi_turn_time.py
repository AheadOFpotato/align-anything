...
class PPOTrainer(RLTrainerBase):  # pylint: disable=too-many-instance-attributes
    ...
    def rl_step(
        ...
        # 计时器上下文管理器
        @contextmanager
        def timer_section(section_name):
        ...
        with timer_section("data_preparation"):
        ...
        with timer_section("mask_processing"):
        ...
        with timer_section("advantage_calculation"):
            with torch.no_grad():
                if self.multi_turn and 'token_level_rewards' in training_batch:
        ...
        with timer_section("actor_forward"):
            logits = self.actor_model(**inference_batch, use_cache=False).logits
            log_probs = gather_log_probabilities(logits[:, :-1], input_ids[:, 1:])
        
        with timer_section("actor_loss_calculation"):
        ...
        with timer_section("actor_backward"):
            self.actor_model.backward(actor_loss)
        
        with timer_section("actor_optimizer_step"):
            self.actor_model.step()

        with timer_section("critic_update"):
            # Update critic model only if available
            if self.use_reward_critic:
                with timer_section("critic_forward"):
                    reward_values = self.reward_critic_model(**inference_batch).scores
                    reward_values = reward_values.squeeze(dim=-1)[:, :-1]
                
                with timer_section("critic_loss_calculation"):
                ...
                with timer_section("critic_backward"):
                    self.reward_critic_model.backward(reward_critic_loss)
                
                with timer_section("critic_optimizer_step"):
        ...
        with timer_section("metrics_calculation"):
            with torch.no_grad():
                if self.multi_turn:
                    mask = loss_mask
        ...
        with timer_section("synchronization"):
            dist.barrier()
        ...
        # 添加耗时统计
        for section_name, elapsed_ms in timing_info.items():
            result[f'timing/{section_name}'] = elapsed_ms
        ...
        # 打印耗时分析（可选，避免日志过多）
        if is_main_process() and self.global_step % 50 == 0:  # 每50步打印一次耗时分析
            print(f"\n--- RL Step {self.global_step} Timing Analysis ---")
            for section_name, elapsed_ms in timing_info.items():
                percentage = (elapsed_ms / total_time) * 100
                print(f"  {section_name}: {elapsed_ms:.2f}ms ({percentage:.1f}%)")
    ...
    def _print_timing_summary(self, current_step, rollout_times, rl_step_times, total_step_times):
        """打印耗时统计摘要"""
        if not rollout_times or not rl_step_times or not total_step_times:
            return
            
        print(f"\n=== Training Timing Summary (Step {current_step}) ===")
        
        # Rollout 统计
        avg_rollout = sum(rollout_times) / len(rollout_times)
        max_rollout = max(rollout_times)
        min_rollout = min(rollout_times)
        rollout_percentage = (avg_rollout / (sum(total_step_times) / len(total_step_times))) * 100
        
        print(f"Rollout:")
        print(f"  Average: {avg_rollout:.2f}ms ({rollout_percentage:.1f}% of step)")
        print(f"  Min/Max: {min_rollout:.2f}ms / {max_rollout:.2f}ms")
        
        # RL Step 统计
        avg_rl_step = sum(rl_step_times) / len(rl_step_times)
        max_rl_step = max(rl_step_times)
        min_rl_step = min(rl_step_times)
        
        print(f"RL Step:")
        print(f"  Average: {avg_rl_step:.2f}ms")
        print(f"  Min/Max: {min_rl_step:.2f}ms / {max_rl_step:.2f}ms")
        print(f"  Total calls: {len(rl_step_times)}")
        
        # 总步骤统计
        avg_total_step = sum(total_step_times) / len(total_step_times)
        max_total_step = max(total_step_times)
        min_total_step = min(total_step_times)
        
        print(f"Total Step:")
        print(f"  Average: {avg_total_step:.2f}ms")
        print(f"  Min/Max: {min_total_step:.2f}ms / {max_total_step:.2f}ms")
        
        # 吞吐量统计
        avg_samples_per_second = (len(rl_step_times) / (sum(total_step_times) / 1000)) if total_step_times else 0
        print(f"Throughput: {avg_samples_per_second:.2f} samples/second")
        
        # 清空历史记录以避免内存增长（保留最近100个记录）
        if len(rollout_times) > 100:
            del rollout_times[:-100]
        if len(rl_step_times) > 100:
            del rl_step_times[:-100]
        if len(total_step_times) > 100:
            del total_step_times[:-100]
